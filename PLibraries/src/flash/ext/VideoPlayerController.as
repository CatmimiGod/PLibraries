package flash.ext
{
	...
	[Event(name="VideoPlayerEvent.UPDATE", type="flash.ext.VideoPlayerEvent")]
	[Event(name="VideoPlayerEvent.CUE_POINT", type="flash.ext.VideoPlayerEvent")]
	...
	public final class VideoPlayerController extends EventDispatcher
	{
		 ...
		 */
		public function VideoPlayerController(updateVideoState:Boolean = false, port:int = 2010)
		{
			_port = port;
		...
		private function onSocketEventHandler(e:Event):void
		{
			switch(e.type)
			{
				...
				case IOErrorEvent.IO_ERROR:
					...
					setTimeout(_socket.connect, 500, "127.0.0.1", _port);
					break;
		...
		private function analyseCuePoint(position:Number):void
		{
			...
			{
				dec = getDecimalPlaces(cuePoints[i]);
				pos = Math.floor(position * dec) / dec;
				cp = Number(cuePoints[i]);
				
				if(cp == pos && _cuePositions[i] != pos)
				{
					_cuePositions[i] = pos;
